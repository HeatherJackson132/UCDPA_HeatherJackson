...
#A list of all of the genres available
genre_list = ["Action","Action-Adventure","Adventure","Board Game","Education","Fighting","Misc","MMO","Music","Party","Platform","Puzzle","Racing","Role-Playing","Sandbox","Shooter","Simulation","Sports","Strategy","Visual Novel"]

#A loop to scrape the data for each genre individually and create a new file
for genre in genre_list:
    ...
    def scrape_all_vg_chartz_videogame_db(results_per_page):
        ...
        while games_left:
            ...
            '&results=' + str(results_per_page) + '&genre=' + str(genre) + '&name=&console=&keyword=&publisher=&order=Sales&ownership\
            ...
            #I further split the above link for the genre for loop. I also updated url to include different data
    ...
    # Run the code to scrape! I did 5,000 rows per page to speed things up.
    df=scrape_all_vg_chartz_videogame_db(5000)
    
    # I changed it to a csv file and updated the name
    df.to_csv(f'HeathersVGchartzDatabase{genre}.csv ', index=False, encoding='utf-8')
    print(f'{genre}File Created!')
